...
export function VendorChatDialog({
  ...
  const { user } = useAuth();
  // If clientUserId is provided explicitly (vendor viewing), use it. Otherwise derive from current user.
  const resolvedClientId = clientUserId ?? (user?.id === vendorUserId ? null : user?.id);
  const { messages, isLoading, sendMessage, markAsRead } = useVendorChat(
    vendorUserId,
    resolvedClientId ?? user?.id ?? null
  );
        ...
        <SheetHeader className="p-4 border-b border-border bg-card/50">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <SheetTitle className="text-foreground text-left">{otherUserName}</SheetTitle>
        ...
        <ScrollArea className="flex-1 p-4">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Inicie a conversa com o vendedor</p>
            </div>
          ) : (
                ...
                <div key={gi}>
                  ...
                  <div className="space-y-3">
                    {group.messages.map((msg, i) => {
                      ...
                      const showAvatar = i === 0 || group.messages[i - 1].sender_id !== msg.sender_id;
                      return (
                        <div key={msg.id} className={cn("flex items-end gap-2", isOwn ? "flex-row-reverse" : "flex-row")}>
                          {!isOwn && showAvatar ? (
                            <Avatar className="w-7 h-7">
                              <AvatarFallback className="text-xs bg-secondary">{msg.sender?.name?.[0] || "?"}</AvatarFallback>
                            </Avatar>
                          ) : <div className="w-7" />}
