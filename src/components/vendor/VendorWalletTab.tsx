...
export function VendorWalletTab() {
  const { user } = useAuth();
  const { vendor, isLoading: loadingVendor } = useVendorProfile();
  ...
  const { data: transactions = [], isLoading: loadingTransactions } = useQuery({
    queryKey: ["vendor-wallet-transactions", vendor?.id],
    queryFn: async () => {
      if (!vendor) return [];
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-destructive/80">
              Carregue sua carteira para manter sua loja ativa.
            </p>
          </div>
        </motion.div>
      )}
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <p className="text-sm text-primary-foreground/70">
            {isBlocked 
              ? "⚠ Sem saldo — sua loja não receberá novos pedidos"
      ...
      {vendor && (
        <WalletRechargeDialog
          ...
          onSuccess={() => {
            ...
            queryClient.invalidateQueries({ queryKey: ["vendor-wallet-transactions"] });
          }}
        />
      )}
    </div>
