...
import { 
  ...
  Loader2, AlertTriangle, CreditCard, Plus
} from "lucide-react";
...
import { WalletRechargeDialog } from "@/components/technician/WalletRechargeDialog";
...
export function DeliveryWalletTab() {
  ...
  const { data: deliveryPerson, isLoading: loadingProfile } = useQuery({
  ...
  const { data: transactions = [], isLoading: loadingTransactions } = useQuery({
    queryKey: ["delivery-wallet-transactions", deliveryPerson?.id],
    queryFn: async () => {
    ...
    <div className="px-4 pb-24">
      <div className="pt-4 mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Seu saldo e comissões</p>
        </div>
        <Button 
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-destructive/80">
              Carregue sua carteira para continuar a receber entregas.
            </p>
          </div>
        </motion.div>
      )}
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <p className="text-sm text-primary-foreground/70">
            {isBlocked 
              ? "⚠ Sem saldo — você não receberá novas entregas"
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card p-4 rounded-2xl">
          <div className="flex items-center gap-2 mb-2">
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  transaction.type === "deposit" ? "bg-success/10" : "bg-destructive/10"
                }`}>
      ...
      {deliveryPerson && (
        <WalletRechargeDialog
          ...
          professionalId={deliveryPerson.id}
          professionalType="delivery"
          onSuccess={() => {
            queryClient.invalidateQueries({ queryKey: ["delivery-person-profile"] });
            queryClient.invalidateQueries({ queryKey: ["delivery-wallet-transactions"] });
          }}
        />
      )}
    </div>
